...
デプロイが完了すると `amplify_outputs.json` が生成されます。フロントエンドはこのファイルから Runtime ARN や Memory ID を自動的に読み取ります。
...
| 会話メモリ | AgentCore Memory によるセッション間の会話履歴保持 |
...
| API Gateway + Lambda + DynamoDB | フィードバック送信時の REST API 呼び出し、Lambda 実行、DynamoDB 書き込み | 月1000件程度のフィードバックなら多くの場合かなり小さくなります。 |
...
| API Gateway + Lambda + DynamoDB | フィードバック1000件程度 | 約 $0.01〜$0.10 |
...
サンプルドキュメントに対する評価結果です。あくまで一問一答のシンプルな質問セットでの結果なので、参考値としてご覧ください。
